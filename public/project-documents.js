(() => {
    ...
    tableBody?.addEventListener("click", async (event) => {
        ...
        if (event.target.closest(".document-delete-btn")) {
            ...
            try {
                const response = await fetch(`/api/project-documents/${row.dataset.id}`, {
                    method: "DELETE",
    ...
    form?.addEventListener("submit", async (event) => {
        ...
        try {
            if (isEdit) {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) {
                    showAlert(formAlert, data.error || "Failed to update document.", "danger");
                    return;
                }
            ...
            if (!projectId) {
                showAlert(formAlert, "Project is required.", "danger");
                return;
            }
